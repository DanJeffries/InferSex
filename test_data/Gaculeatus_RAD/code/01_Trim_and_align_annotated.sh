...
WD=/storage/scratch/iee_evol/sascha/SIAS/test_data/G_aculeatus_RAD ## set working directory

ITER_FILE=/storage/homefs/dj20y461/Infersex/InferSex/test_data/Gaculeatus_RAD/code/sample_paths.txt  ## the job array will iterate over this file, which contains paths to each sample fastq files. 
...
SAMPLE_NAME=$(echo $SAMPLE_FASTQ_PATH | rev | cut -f1 -d'/' | rev | cut -f1 -d'.') ## get just the sample name from the file path, for cleaner file naming later on. 

######### Aligning ##############################################################################################################################################################################################

# Once the data has been trimmed, we can align it to the genome. 

DATA_DIR=/storage/research/iee_evol/Sascha/sascha/SIAS/test_data/G_aculeatus_RAD
SAMPLE_R1=$DATA_DIR/$SAMPLE_NAME.fastq
...
BAM_COORDSORT=${BAMDIR}/${SAMPLE_NAME}.filtered.coordsorted.bam ##
...
samtools index -c -@ 20 $BAM_COORDSORT
